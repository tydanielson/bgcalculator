...
(function(){
	angular.module('wsi')
		...
		$scope.getStockQuote = function() {
			...
			var ticker = $scope.ben.ticker.toUpperCase();
			console.log(ticker);

			$http.get('/stock/quote/' + ticker)
				.then(function(res){
					//console.log(res.data);
					if (res.data.quote === null) {
						var url = 'https://services.last10k.com/v1/company/' + ticker + '/quote'
						$http.get(url)			
							.then(function(data){
		...
		$scope.getRatios = function() {
			...
			$http.get('/stock/ratio/' + ticker)
				.then(function(res){
					if(res.data.stock === null){
						var url = 'https://services.last10k.com/v1/company/' + ticker + '/ratios'
						$http.get(url)			
							.then(function (data) {
